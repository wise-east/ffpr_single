...
def get_data_loaders(args, tokenizer):
    ...
    # to test everything is functional, run with smaller set 
    if args.debug: 
        data = {dataset_name: dataset[:10] for dataset_name, dataset in data.items()}

    datasets = {"train": defaultdict(list), "valid": defaultdict(list)}
    for dataset_name, dataset in data.items():
        for sample in dataset: 

            instance = build_input_from_segments(sample['src'], sample['target'], tokenizer, lm_labels=True)
            if instance: 
...
def train():
    ...
    parser.add_argument("--data_path", type=str, default="../data/mlb_data.json", help="Path or url of the dataset. If empty download from S3.")
    parser.add_argument("--data_cache", type=str, default='../data/mlb_data_cache', help="Path or url of the dataset cache")
    parser.add_argument("-mc", "--model_checkpoint", type=str, default="openai-gpt", help="Path, url or short name of the model")
    ...
    parser.add_argument("--debug", type=bool, default=False, help="Run through small dataset to test if entire code is functional")
    ...
    tokenizer = tokenizer_class.from_pretrained(args.model_checkpoint, cache_dir='../')


    model_class = GPT2DoubleHeadsModel if "gpt2" in args.model_checkpoint else OpenAIGPTDoubleHeadsModel
    model = model_class.from_pretrained(args.model_checkpoint, cache_dir='../')
    ...
    def update(engine, batch):
        ...
        (lm_loss),*_ = model(
            input_ids, token_type_ids=token_type_ids,lm_labels=lm_labels
        )
        loss = lm_loss
        if args.fp16:
